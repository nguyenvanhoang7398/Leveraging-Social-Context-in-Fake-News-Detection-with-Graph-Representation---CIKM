...
% Hoang - CR: I removed the author's state to save some space.
% Preslav: I have put it back, as otherwise we have the huge empty space on the left.
   \state{Singapore}
  \postcode{117417}
}
...
% MinCR: your first page looks like it is formatted incorrectly.  Check formatting -- imbalanced columns -- too little text in the left column.
% Hoang - CR: reformatted
...
%\footnote{\scriptsize{\url{https://www.facebook.com/help/572838089565953?helpref=faq_content}}} 
...
%\footnote{\scriptsize{\url{http://snopes.com}}} 
FactCheck,
%\footnote{\scriptsize{\url{http://www.factcheck.org}}} 
PolitiFact,
%\footnote{\scriptsize{\url{http://politifact.com}}}
and Full Fact.
...
%Kaz-CR: Rewrote a little bit 
%Recent research has taken 
...
%Kaz-CR: Edited
%However, there was insufficient emphasis on the quality of representation, limited modeling of entities and their interactions, and little focus on minimally supervised settings. 
...
%Kaz-CR: Edited
%where nodes are the social entities and the edges represent the interactions between them. 
...
% MinCR: caption needs to better describe the 5 right columns.  
\caption{Comparison between representation learning frameworks for social entities (1. users, 2. news, 3. sources) and interactions (4. user-user friendship, 5. user--news engagement, 6. source--news publication, 7. source--source citation) on whether they consider engagement time, graph modelling of social context, deep learning, inductiveness, and representation learning.}
  ...
  \toprule
    ...
    \midrule 
    % MinCR: changed first two fields back to left alignment.  
    % Suggest you drop the periods after numbers in second field, looks distracting.
    % Hoang - CR: Thanks, I have dropped
...
%Hoang - CR: I intended this to be a navigation paragraph to improve coherence. We can omit this if space contrained.
...
%The motivations for our work come from %two primary areas: the following two areas: 

%Kaz-CR: Better to insert "\noindent" as "1. Contextual Fake News Detection" and "2. Graph Neural Network" are a kind of subsections.   
...
%Kaz-CR: Better to use "not" instead of "less" to differentiate our work from others
%As a result, they are less robust 
...
%Kaz-CR: Can write the following sentence more simply
%Table~\ref{table:literature_review} offers a summarized comparison of these approaches.
Table~\ref{table:literature_review} compares these approaches.

\begin{table*}[tbh]
  \centering
  \small
  % MinCR: changed caption; please check
  % Hoang - CR: changed
...
\begin{enumerate}
    ...
    %Kaz-CR: Used a subscript
    where each $a_{i}$ $(i=1,2,...)$ is modeled as a feature vector $\boldsymbol{x}_{a}$.
    \item $S=\{s_1, s_2,...\}$ is the list of \textbf{news sources}, where each source $s_j$ $(j=1,2,...)$ has published at least one article in $A$, %and each $s$ 
...
%    \item $U=\{u_1, u_2,...\}$ be the list of \textbf{social users}, where each user has engaged in spreading any article in $A$ or has a connection with any user in $U$, where each $u$ is modeled as a feature vector $\boldsymbol{x}_{u}$.
    ...
    \item $E=\{e_1, e_2,...\}$ is the list of {\bf interactions}, and each interaction $e=\{v_1, v_2, t, x_e\}$ is modeled as a relation between two entities $v_1, v_2\in A\cap S\cap U$ at time $t$; %where 
...
%Kaz-CR: Edited. Please check the following: 
%whereas the division of \textit{support} 
...
% Hoang - CR: omit this due to limited space
...
%We enrich the semantic representation of the tokens by weighing them with pre-trained word embeddings from GloVe~\cite{pennington2014glove} to construct $\boldsymbol{v}^s_a$. Finally, we concatenate $\boldsymbol{v}^t_a$ and $\boldsymbol{v}^s_a$ to form the news article feature vector $\boldsymbol{x}_a$.
...
\textbf{News Sources.} 
% Hoang - CR: omit this due to space constraint
...
% MinCR: how is a directory different from a section?  E.g., the below descriptions characterising About Us?
...
%within their \emph{About Us} section.
% Hoang - CR: this is actually speculative
% in order to protect themselves from being censored by social media platforms. 
...
friends. 
%Kaz-CR: Specify reference ID here not just "recent work" [38], [39] or both?
%Recent work by Shu {\it et al.}~
% MinCR: missing cite BUG
...
% \todo{Hoang: elaborate more on the figure caption of FANG}
...
% Hoang - CR: Abbreviate as we have defined RNNs above
...
% MinCR: Global: use \top instead of T for matrix transpose.
...
%Kaz-CR: I have added footnote IDs shown in page 1 for better reference
%Kaz-CR2: I have removed footnote IDs ($^{2}$ and $^{4}$) as they are commented out in page 1.  
%--- Snopes$^{2}$ and Politifact$^{4}$. 
...
%Kaz-CR: Actually, it is better to cite the original paper here. 
...
%Kaz-CR: I think that $t$ is better than just "t". 
...
%Kaz-CR: The order of the difference value seems to be wrong. 
% I exchanged them. 
%by .0.0386 and .0501, respectively: 
...
%Kaz-CR: I have added "(without $time(e)$)" to make "-t" more clearer
\begin{table}[tb]
    \centering
    \small
...
%Kaz-CR: Edited the following sentence
...
%Kaz-CR: "0.6123 at 10\%" should be "0.5918 at 10\%"? 
%0.6123 at 10\%, 
...
%Kaz-CR: In GCN, "6455" at 30% should be "0.6445"
\begin{table}[t]
    \centering
    \small
...
%Kaz-CR: Edited the following sentence, please check it. 
%and thus the higher the quality of the representation. 
...
%Kaz-CR2: I have removed footnote ID ($^{4}$) as it is commented out in page 1, but added actual footnote from page 1 for "PolitiFact" as "Media Bias/Fact Check" has its URL here. 
%and PolitiFact$^{4}$.
...
%Kaz-CR: Better to show the score as "0.XYZ" as it is unclear if the score starts with decimal point, "." 
...
%\footnote{\scriptsize{\url{https://www.cnsnews.com/}}} 
...
% Kaz-CR: Slightly edited
%Yet, GCN poorly differentiates 
...
%\footnote{\scriptsize{\url{https://news.yahoo.com/}}}, 
...
%\footnote{\scriptsize{\url{https://www.nytimes.com/}}}, 
\textit{washingtonpost}
%\footnote{\scriptsize{\url{https://www.washingtonpost.com/}}}
...
%\footnote{\scriptsize{\url{https://www.buzzfeednews.com/}}},
\textit{nypost},
%\footnote{\scriptsize{\url{https://www.nypost.com/}}} 
...
%Kaz-CR: Better to keep the order, in other words, 
% "D (proof reader) comes first and then "E (tech community)". 
%a tech community and a proof reader. 
